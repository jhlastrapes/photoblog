        ...
        for(var j = 0; j < images.length; j++){
          ...
          if(("mImg"+j) === currentImageID){
            ...
            if(currentImageID !== ("mImg"+(images.length-1)) && (touchStartX-touches[i].pageX) > 0){
              ...
              nextDOM.style.left = ((touches[i].pageX-touchStartX + 50) + "px");  // once movemnt is > 0px
            }
            // right swipe (if not 1st image)
              ...
              prevDOM.style.left = ((touches[i].pageX-touchStartX-window.outerWidth-50) + "px");  // once movemnt is > 0px
            };
          };
        };
  ...
  function handleTouchEnd(evt) {
    ...
    for (var i = 0; i < touches.length; i++) {
      ...
      if (idx >= 0) {
        ...
        var fadeTime = 0.5 // transition time (in seconds)
        ...
        else if (Math.abs(touchStartX-touchEndX) < 150){
          for(var j = 0; j < images.length; j++){
            // short left swipe
